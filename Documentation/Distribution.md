...
## Public installer: 0.1.6

The current public installer is **Yap 0.1.6 (build 7)**. [Download the early preview](https://github.com/grinich/yap/releases/tag/v0.1.6). It includes native audio/video menus, camera effects, expanded meeting chat, a menu-bar schedule, gallery photos and sharing improvements. [Release notes](ReleaseNotes/v0.1.6.md) and the [exact release evidence](ZoomReview-0.1.6.md) describe the supported features and verification limits. Publishing on GitHub changes installer availability, not provider approval. Zoom's functional review still requires follow-up; Google sign-in is in production but unverified.

The next review candidate, **0.1.7 (build 8)**, uses the production OAuth client through a fixed HTTPS callback. It needs its own packaged artifact and live authorization verification; the published 0.1.6 binary still uses the earlier public-client flow. The [test plan](Zoom-Test-Plan.md) distinguishes the two configurations.

## Historical September 7 submission and approval requirements
...
Google Calendar is optional. Every current build bundles the same canonical Yap desktop client from `Sources/YapCalendar/Resources/GoogleOAuth.plist`. Users choose **Connect Google Calendar** and sign in directly. No developer setup, imported client, or separate production client is required. The historical notarized 0.1.3 candidate predates this packaging change. The project is now **In production**, but Google verification remains pending. Google’s unverified-app warning and user cap still apply. See [Google verification](Google-Verification.md) for the current review checkpoint.
