...
The **Recordings** button beside the window controls opens an animated left sidebar. Select a meeting to play its first available video in the main view. The native player provides playback, seeking, volume, and full-screen controls. Meetings with several video views or recording segments offer a video picker. Closing the library pauses inline playback while retaining its position.
...
**Copy link** and **Save video…** sit immediately beside the recording title as matching neutral icon buttons, with the date below. The Chat/Transcript tabs and pane toggle remain at the right edge. Long titles truncate to keep both actions visible in narrow players.
...
The first load searches the current calendar month and the previous two months. **Load earlier month** continues backward, including through empty months. Search filters the meetings already loaded; it does not search the entire Zoom account. Reopening the recordings view refreshes all loaded months without changing the selection or player. Earlier-month requests wait behind refresh, and their entire button row is clickable. Dates, titles, loading failures, and meetings with no completed video remain visible in the library.

Switching video views keeps the current elapsed position, pause state, and playback speed. A shorter view clamps to its final frame. The previous frame stays visible while the new view's metadata loads, and playback resumes after an exact seek. Rapid switches carry the original position through unfinished loads. The three most recently used views stay warm for the current meeting, with an 8 MiB memory range cache per view; changing meetings or changing accounts discards that cache.

The visible speed menu beside the layout button offers 1×, 1.25×, 1.5×, 1.75×, 2×, 2.25×, 2.5×, and 2.75×. **S** cycles speed, **V** cycles layouts, **Left/Right** jog ten seconds, and **F** toggles fullscreen for the active recording window. Choosing a speed does not start a paused recording. The setting applies when pressing Space or the native play button and follows the recording when it opens in a dedicated window. The native player's speed menu and the visible menu stay synchronized.
...
Chat loads when a recording becomes active or the pane opens, remains in memory, and moves with a dedicated player window. Account changes and dedicated player window closure cancel pending chat work. Downloads use the existing recording authorization, validate text responses, and enforce an 8 MiB limit. Parsing and merging run off the UI actor. Files from the same meeting are merged without duplicating overlapping exports or losing identical repeated messages within one export.
...
The right pane has **Chat** and **Transcript** tabs. The transcript tab displays the recording's spoken WebVTT transcript with speaker names when Zoom supplies them. Playback highlights cues only during their start/end interval and follows the current passage; scrolling or searching pauses following until **Follow playback** is selected. Clicking a passage or its timestamp seeks without changing play/pause state or speed.

Search matches text and speakers without case or accent sensitivity. Matching words are highlighted; the previous/next controls and Return navigate matching passages and seek the current video. Search stays within the open recording. Tab switches, video-layout changes, nondestructive refreshes, and transfers to dedicated windows retain transcript content and interaction state. A transcript-only recording opens its transcript pane automatically; an explicitly closed pane stays closed across layout changes and window transfer.

Text remains selectable. **Copy passage** in a passage's context menu includes its timestamp and speaker; the footer's **Copy transcript** copies all passages, independent of the search. **Download transcript** saves UTF-8 TXT or WebVTT through a save sheet initially showing Downloads. WebVTT export uses the merged transcript timeline. Exported files are written only at the chosen destination; otherwise transcript text remains in memory and is cleared with the account or player.

The loader fetches only the transcript text using recording OAuth authorization and the existing trusted redirect policy. It accepts VTT/plain text, enforces an 8 MiB bound, checks incomplete downloads, rejects HTML/login responses, and refreshes expired authorization once. The parser handles Zoom speaker prefixes, WebVTT voice tags, multiline speech, cue settings and metadata, and deterministic cue identities. Failed requests can be retried.

Transcript clocks are relative to the recording file. When both transcript and video provide `recording_start`, their difference aligns cues with the selected video; missing anchors use the original VTT clock. Multiple files merge onto a common transcript timeline. Pauses or edits inside an MP4 can still require a timing notice, since the API does not describe those internal cuts. Recordings without a completed transcript show an availability message. [Zoom transcript format and availability](https://support.zoom.com/hc/en/article?id=zm_kb&sysparm_article=KB0064927).

Transcript validation on September 7, 2026: all 608 tests in 73 suites passed with the real Zoom SDK enabled, and the signed build was installed and restarted. Live transcripts loaded for Homepage jam and AuthKit marketing. Searching `weekend` found two passages and navigation sought the matching video positions. During playback, the highlight and scroll advanced with the spoken cues. Switching video layouts retained the active passage and speed; opening a dedicated window retained the transcript tab, query, 1.25× speed, and paused 00:30 position. Switching Chat → Transcript also kept that position. The pane fit at the minimum window size. Copy passage included its timestamp and speaker, and whole-transcript copy included all passages. TXT and WebVTT save sheets initially showed Downloads; both saved complete 702-passage transcripts despite the active search.
