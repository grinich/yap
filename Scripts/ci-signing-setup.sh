...
umask 077
keychain="$RUNNER_TEMP/yap-release.keychain-db"
password="$(openssl rand -hex 32)"
echo "::add-mask::$password"
echo "YAP_NOTARY_KEYCHAIN=$keychain" >> "$GITHUB_ENV"
echo "YAP_UPDATE_PRIVATE_KEY_FILE=$RUNNER_TEMP/sparkle-key.txt" >> "$GITHUB_ENV"
...
xcrun notarytool store-credentials yap-ci --key "$RUNNER_TEMP/notary-key.p8" \
