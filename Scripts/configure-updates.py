...
def configure(info, env):
    feed = env.get("YAP_UPDATE_FEED_URL", "")
    key = env.get("YAP_UPDATE_PUBLIC_KEY", "")
    if not feed and not key:
        if env.get("YAP_RELEASE") == "1":
            raise ValueError("Release builds require YAP_UPDATE_FEED_URL and YAP_UPDATE_PUBLIC_KEY.")
        return info
    url = urlsplit(feed)
