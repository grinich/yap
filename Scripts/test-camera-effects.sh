...
YAP_CAMERA_TEST_WORK="$(mktemp -d "${TMPDIR:-/tmp}/yap-camera-effects-tests.XXXXXX")"
...
xcrun clang -fobjc-arc -fmodules -mmacosx-version-min=26.0 \
    ...
    "$YAP_ROOT/Tests/YapZoomBridgeTests/CameraEffectsLifecycleTests.m" \
    -o "$YAP_CAMERA_TEST_WORK/camera-effects-tests"

"$YAP_CAMERA_TEST_WORK/camera-effects-tests" "$YAP_CAMERA_TEST_WORK"
