...
class ReleaseConfigurationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.env = {"ZOOOM_RELEASE": "1", "ZOOOM_UPDATE_FEED_URL": "https://example.com/appcast.xml",
                    "ZOOOM_UPDATE_PUBLIC_KEY": self.key}

    def test_release_cannot_silently_disable_updates(self):
        with self.assertRaises(ValueError):
            configure({}, {"ZOOOM_RELEASE": "1"})
        self.assertEqual(configure({"CFBundleName": "Zooom"}, {}), {"CFBundleName": "Zooom"})
    ...
    def test_invalid_keys_and_insecure_feeds_fail_closed(self):
        for url in ["http://example.com/feed", "file:///tmp/feed", "https://name:secret@example.com/feed", "https://"]:
            with self.subTest(url=url), self.assertRaises(ValueError):
                configure({}, {**self.env, "ZOOOM_UPDATE_FEED_URL": url})
        for key in ["", "not-base64", base64.b64encode(bytes(31)).decode()]:
            with self.subTest(key=key), self.assertRaises(ValueError):
                configure({}, {**self.env, "ZOOOM_UPDATE_PUBLIC_KEY": key})

    def test_release_configuration_requires_signed_feed_and_private_profile(self):
        info = configure({"CFBundleIdentifier": "com.grinich.zooom"}, self.env)
        ...
        self.assertEqual(info["CFBundleIdentifier"], "com.grinich.zooom")

    def test_unsigned_misdirected_and_mismatched_appcasts_fail(self):
        with tempfile.TemporaryDirectory() as directory:
            archive = Path(directory) / "Zooom-macOS.zip"
            ...
            feed = f'''<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle"><channel><item>
                ...
                <enclosure url="https://github.com/example/releases/releases/download/v0.1.1/Zooom-macOS.zip"
