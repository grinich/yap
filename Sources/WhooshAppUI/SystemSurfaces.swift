...
public struct WhooshMenuBarView: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack { WhooshMark(size: 28); Text("Zooom").font(.headline); Spacer(); if model.isPreview { Text("Preview").font(.caption).foregroundStyle(.orange) } }
            if model.activeCall {
            ...
            Divider()
            Button(model.activeCall ? "Return to meeting" : "Open Zooom") { showMain() }.buttonStyle(.plain)
            Button("Quit Zooom") { NSApplication.shared.terminate(nil) }.buttonStyle(.plain).foregroundStyle(.secondary)
        }.padding(22).frame(width: 280).tint(WhooshTheme.accent)
    }

    private func showMain() { openWindow(id: "main"); NSApplication.shared.activate() }
}

public struct WhooshCommands: Commands {
    ...
    public var body: some Commands {
        CommandGroup(after: .newItem) {
            Button("Open Zooom") { WhooshSystemActions.request(.openWhoosh) }.keyboardShortcut("0", modifiers: .command)
...
@MainActor
public final class WhooshApplicationDelegate: NSObject, NSApplicationDelegate {
    public weak var model: WhooshModel?
    ...
    private lazy var windowPresenter = WhooshMainWindowPresenter(actions: .init(
        ...
        restoreWindow: { [weak self] in
            ...
            return WhooshMainWindowState(isVisible: window.isVisible,
    ...
    public func configure(model: WhooshModel, openMainWindow: @escaping () -> Void) {
        self.model = model
        self.openMainWindow = openMainWindow
        if sharingOverlayController == nil {
            sharingOverlayController = WhooshSharingOverlayController(model: model) { [weak self] in
                self?.presentMainWindow()
            }
        }
        if menuBarController == nil {
            menuBarController = WhooshMenuBarController(model: model,
    ...
    public func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        let alert = NSAlert()
        alert.messageText = "Leave the meeting and quit Zooom?"
        ...
        Task {
            ...
            if !ended { model.error = "Zoom hasn’t confirmed that the meeting ended. Zooom is still open; try leaving again." }
...
struct WindowBehavior: NSViewRepresentable {
    ...
    @MainActor final class Coordinator: NSObject, NSWindowDelegate {
        ...
        private let closeButton = WhooshWindowCloseButton(frame: .zero)
        ...
        func attach(to window: NSWindow) {
            ...
            NotificationCenter.default.post(name: whooshMainWindowAttached, object: model)
        }
        func detach() {
            closeButton.removeFromSuperview()
