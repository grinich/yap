...
/// A single light, frosted backdrop that samples windows behind Whoosh.
...
struct WhooshWindowBackdrop: NSViewRepresentable {
...
struct WhooshMark: View {
    var size: CGFloat = 40
    var body: some View {
        Group {
            if let url = Bundle.main.url(forResource: "WhooshIcon", withExtension: "png"), let icon = NSImage(contentsOf: url) {
                ...
                Text("Z")
                    ...
                    .background(WhooshTheme.accent.gradient, in: RoundedRectangle(cornerRadius: size * 0.25))
            }
...
struct WhooshGlassSurface: ViewModifier {
...
extension View {
    ...
    func whooshGlassSurface(cornerRadius: CGFloat = 20) -> some View {
        modifier(WhooshGlassSurface(cornerRadius: cornerRadius))
    }
}
