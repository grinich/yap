...
public struct WhooshRootView: View {
    @Bindable var model: WhooshModel
    @Environment(\.openWindow) private var openWindow
    @Environment(\.openSettings) private var openSettings
    @Environment(\.accessibilityReduceTransparency) private var reduceTransparency
    private let applicationDelegate: WhooshApplicationDelegate?
    public init(model: WhooshModel, applicationDelegate: WhooshApplicationDelegate? = nil) {
        self.model = model
    ...
    public var body: some View {
        ...
        .background {
            if !reduceTransparency {
                WhooshWindowBackdrop().ignoresSafeArea()
        ...
        VStack(alignment: .leading, spacing: compact ? 16 : 22) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Label("Up Next", systemImage: "video")
                    .font(.subheadline.weight(.semibold)).foregroundStyle(WhooshTheme.accent)
    ...
    private func welcome(compact: Bool) -> some View {
        VStack(alignment: .leading, spacing: compact ? 18 : 24) {
            HStack(spacing: compact ? 12 : 18) {
                WhooshMark(size: compact ? 38 : 60)
                VStack(alignment: .leading, spacing: 5) {
                    Text("Meetings, with room to breathe.")
                        .font(.system(size: compact ? 19 : 22, weight: .semibold))
                    Text("Your next conversation. One click away.")
            ...
            HStack(alignment: .top, spacing: 18) {
                if !compact {
                    Image(systemName: "calendar").font(.system(size: 24, weight: .light))
                        .foregroundStyle(WhooshTheme.accent).frame(width: 40)
                }
            ...
            VStack(spacing: 0) {
                ForEach(items) { event in
                    HStack(spacing: compact ? 10 : 16) {
                        if !compact {
                            RoundedRectangle(cornerRadius: 2).fill(WhooshTheme.accent.opacity(0.65)).frame(width: 3, height: 32)
                        }
                        VStack(alignment: .leading, spacing: 4) {
...
struct JoinMeetingSheet: View {
    ...
    var body: some View {
        ...
        }.padding(30).frame(width: 460).tint(WhooshTheme.accent)
