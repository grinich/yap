...
enum WhooshMenuBarPrimaryAction: Equatable {
    case openWhoosh, joinMeeting, returnToMeeting, stopSharing
    var title: String {
        switch self {
        case .openWhoosh: "Zooom"
...
final class WhooshMenuBarActionHandler {
    ...
    init(model: WhooshModel, now: @escaping () -> Date = { .now },
    ...
    var canPerformPrimaryAction: Bool {
        ...
        return primaryAction == .openWhoosh || model.activeCall || !isPerforming
    }

    func performPrimaryAction(expectedAction: WhooshMenuBarPrimaryAction? = nil, expectedMeetingID: String? = nil) async {
        ...
        if primaryAction == .openWhoosh { toggleMainWindow(); return }
    ...
    func openWhoosh() { openMainWindow() }
...
@MainActor
final class WhooshMenuBarController: NSObject {
    private let model: WhooshModel
    private let openMainWindow: () -> Void
    private let actions: WhooshMenuBarActionHandler
    private let statusItem: NSStatusItem
    private let pill = WhooshMenuBarPill(frame: .zero)
    private var actionTask: Task<Void, Never>?
    private var eligibilityTimer: Timer?
    private var stopped = false

    init(model: WhooshModel, toggleMainWindow: (() -> Void)? = nil, openMainWindow: @escaping () -> Void) {
        self.model = model
        self.openMainWindow = openMainWindow
        actions = WhooshMenuBarActionHandler(model: model, toggleMainWindow: toggleMainWindow,
                                            openMainWindow: openMainWindow)
        statusItem = NSStatusBar.system.statusItem(withLength: NSStatusItem.variableLength)
        super.init()
        statusItem.autosaveName = "WhooshJoinMeeting"
    ...
    private func renderAndObserve() {
        ...
        withObservationTracking {
            ...
            else if action == .returnToMeeting { tooltip = "Return to your Zooom meeting" }
            else if action == .joinMeeting { tooltip = title + " · Right-click for more options" }
            else { tooltip = "Show or hide Zooom · Right-click to join with a link" }
    ...
    private func showContextMenu() {
        ...
        let open = NSMenuItem(title: model.activeCall ? "Return to meeting" : "Open Zooom", action: #selector(openWhoosh), keyEquivalent: "")
        open.target = self
        menu.addItem(open)
        ...
        menu.addItem(join)
        menu.addItem(.separator())
        let quit = NSMenuItem(title: "Quit Zooom", action: #selector(quitWhoosh), keyEquivalent: "")
        quit.target = self
        menu.addItem(quit)
    ...
    @objc private func openWhoosh() {
        DispatchQueue.main.async { [weak self] in self?.actions.openWhoosh() }
    }
    @objc private func joinWithLink() {
        DispatchQueue.main.async { [weak self] in self?.actions.joinWithLink() }
    }
    @objc private func quitWhoosh() { NSApplication.shared.terminate(nil) }
}
...
@MainActor
private final class WhooshMenuBarPill: NSView {
