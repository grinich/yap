...
public protocol WhooshCalendarServing: Sendable {
...
extension GoogleCalendarClient: WhooshCalendarServing {}
...
@MainActor
public struct WhooshReminderActions {
    ...
    public static var live: Self {
        ...
        return Self(requestAuthorization: { try await service.requestAuthorization() },
                    synchronize: { items, lead in
                        ...
                        if !report.authorizationGranted { throw WhooshReminderPermissionError.notificationsNotAllowed }
                    },
...
enum WhooshReminderPermissionError: Error { case notificationsNotAllowed }

@MainActor @Observable
public final class WhooshModel {
    ...
    private var calendarClient: any WhooshCalendarServing
    @ObservationIgnored private let reminders: WhooshReminderActions
    ...
    @ObservationIgnored private let makeConfiguredCalendarClient: (GoogleOAuthConfiguration) -> any WhooshCalendarServing
    ...
    public init(preview: Bool = ProcessInfo.processInfo.arguments.contains("--preview"),
                ...
                calendarClient: (any WhooshCalendarServing)? = nil,
                reminders: WhooshReminderActions? = nil,
                saveGoogleConfiguration: ((Data) throws -> Void)? = nil,
                loadGoogleConfiguration: (@Sendable () throws -> GoogleOAuthConfiguration?)? = nil,
                makeConfiguredCalendarClient: ((GoogleOAuthConfiguration) -> any WhooshCalendarServing)? = nil) {
        ...
        self.saveGoogleConfiguration = saveGoogleConfiguration ?? WhooshConfigurationStore.saveGoogle
    ...
    private static func makeCalendarClient(configuration: GoogleOAuthConfiguration? = nil) -> GoogleCalendarClient {
        let cache = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            .appending(path: "Whoosh", directoryHint: .isDirectory).appending(path: "agenda.json")
    ...
    private func performRefresh(client: any WhooshCalendarServing, selected: [GoogleCalendar], generation current: UUID, selection: UUID, reloadCalendars: Bool) async -> Bool {
        let now = Date()
        do {
    ...
    public func importGoogleConfiguration(from url: URL) async {
        do {
            ...
            let configuration = try WhooshConfigurationStore.parseGoogle(data)
    ...
    public func openReminderNotificationSettings() {
        ...
        if !isWaitingForNotificationSettings {
            reminderPermissionError = "Open System Settings, choose Notifications, then select Zooom."
        }
    }
    ...
    public func setReminders(_ enabled: Bool) async {
        ...
        if enabled {
            ...
            do {
                ...
                remindersEnabled = false
                showReminderPermission = true
                if error is WhooshReminderPermissionError {
                    reminderAuthorizationStatus = .denied
                } else {
                    reminderPermissionError = "Zooom couldn’t enable notifications. Please try again."
                }
            }
        } else {
            remindersEnabled = false
    ...
    public func handleSystemActions() async {
        while let action = WhooshSystemActions.takePendingAction() {
    ...
    public func handleSystemAction(_ action: WhooshSystemAction) async {
        switch action {
            case .openWhoosh, .showUpcomingMeetings: break
...
enum WhooshConfigurationStore {
    private static let service = "app.whoosh.personal.configuration"
