...
private actor OAuthLoopbackListener {
    ...
    private let queue = DispatchQueue(label: "app.whoosh.google-oauth-loopback")
    ...
    private func accept(_ connection: NWConnection) async {
        ...
        do {
            ...
            do {
                ...
                await respond(connection, status: "200 OK", message: "Sign-in received. You can return to Zooom.")
                finish(.success(code))
            } catch GoogleCalendarError.authorizationDenied {
                await respond(connection, status: "200 OK", message: "Calendar access was not granted. You can return to Zooom.")
                finish(.failure(GoogleCalendarError.authorizationDenied))
            }
        } catch {
            await respond(connection, status: "400 Bad Request", message: "This sign-in response could not be verified.")
        }
    }

    private func respond(_ connection: NWConnection, status: String, message: String) async {
        let body = "<!doctype html><html><meta name=\"viewport\" content=\"width=device-width\"><title>Zooom</title><body><p>\(message)</p></body></html>"
