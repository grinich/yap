...
public final class LaunchAtLoginService {
    ...
    public func setEnabled(_ enabled: Bool) async {
        ...
        guard dependencies.isApplicationBundle() else {
            errorMessage = "Open the installed Zooom app to change its login setting. This setting isn’t available when running the executable outside its app bundle."
            return
        }
        ...
        if let operationError {
            errorMessage = "macOS couldn’t turn \(enabled ? "on" : "off") Open Zooom at login. \(operationError.localizedDescription)"
        } else {
            errorMessage = "macOS hasn’t applied this login setting. Try again, or check Zooom in Login Items in System Settings."
        }
    }
