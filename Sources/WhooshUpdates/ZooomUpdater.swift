...
struct UpdateConfiguration {
    ...
    init?(bundle: Bundle) {
        guard bundle.bundleIdentifier == "com.grinich.zooom",
...
@MainActor
public final class ZooomUpdater: NSObject, ObservableObject, SPUUpdaterDelegate {
    ...
    public func updater(_ updater: SPUUpdater, mayPerform updateCheck: SPUUpdateCheck) throws {
        guard gate.canInstall else {
            throw NSError(domain: "com.grinich.zooom.updates", code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Zooom will check for updates after your meeting."])
        }
    }

    // This is called even when Sparkle resumes a staged update on a later launch.
    // Vetoing leaves Zooom running; the user can install after the call ends.
    public func updaterShouldRelaunchApplication(_ updater: SPUUpdater) -> Bool { gate.canInstall }
}

public struct ZooomUpdateCommands: Commands {
    @ObservedObject private var updater: ZooomUpdater
    private let meetingActive: Bool

    public init(updater: ZooomUpdater, meetingActive: Bool) {
