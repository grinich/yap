        ...
        return Button { model.selectFromList(meeting) } label: {
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: count > 0 ? "play.rectangle" : "clock")
                    ...
                    .foregroundStyle(selected ? YapTheme.accent : .secondary)
            ...
            .background(selected ? YapTheme.accent.opacity(0.12) : .clear, in: RoundedRectangle(cornerRadius: 10))
            .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
        ...
        Group {
            if let meeting = model.selectedMeeting {
                ...
                emptyPlayer(title: "Pick a recording", subtitle: "Your meetings, ready to replay. Select a recording from the library to start watching.")
                    .overlay(alignment: .top) {
                        YapWindowDragSurface().frame(height: headerHeight)
                    }
            }
        }
        ...
        return ZStack(alignment: .topLeading) {
            ...
            HStack(spacing: 0) {
                ...
                HStack(spacing: 0) {
                    ...
                    .yapIconHover(isSelected: model.chat.isPresented)
        ...
        HStack(spacing: 12) {
            ...
            if model.selectedFile != nil, !model.isPreview {
                Button(action: saveVideo) {
                    ...
                    .buttonStyle(.plain).yapIconHover().foregroundStyle(.primary)
