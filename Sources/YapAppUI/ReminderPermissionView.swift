...
/// Yap explains the choice here. macOS retains its own permission controls.
struct ReminderPermissionView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            if model.isChangingReminders {
                ...
                Text("Yap will check again when you return.")
                    .font(.callout).foregroundStyle(.secondary)
            }
    ...
    private var explanation: String {
        if isDenied {
            "Notifications are off for Yap. In System Settings, select Yap and turn on Allow Notifications."
        } else {
            "Get a reminder before your Zoom meetings, with a shortcut back to Yap. macOS will ask you to allow notifications."
        }
    }
}
