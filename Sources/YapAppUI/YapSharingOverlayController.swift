...
struct YapSharingChatPlacement {
    ...
    mutating func frameForPresentation(currentFrame: CGRect?, on screen: CGRect) -> CGRect {
        ...
        guard let currentFrame, lastPresentedScreen == screen else {
            return YapSharingOverlayLayout.chatFrame(in: screen)
        }
        return currentFrame
    }
}

struct YapSharingStopTransition {
...
@MainActor
public final class YapSharingOverlayController: NSObject, NSWindowDelegate {
    ...
    private var chatPlacement = YapSharingChatPlacement()
    private var stopTransition = YapSharingStopTransition()

    public init(model: YapModel, openMainWindow: @escaping () -> Void = {}) {
        self.model = model
    ...
    private func updateStrip(on fallbackScreen: NSScreen) {
        let participants = YapSharingOverlayLayout.participants(from: model.meeting.visibleParticipants)
        ...
        let proposed = customStripFrame ?? YapSharingOverlayLayout.stripFrame(in: screen.visibleFrame,
    ...
    private func updateChat(on screen: NSScreen) {
        guard YapSharingOverlayLayout.shouldPresentChat(isPresenting: model.sharingPresentation.isPresenting,
    ...
    private func dismissPanels() {
        ...
        chatPlacement = YapSharingChatPlacement()
