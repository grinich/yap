...
@MainActor
struct YapWindowPointerPresence: NSViewRepresentable {
    var onChange: (Bool) -> Void
    var onKeyboardActivity: () -> Void = {}

    func makeNSView(context: Context) -> YapWindowPointerTrackingView {
    ...
    func updateNSView(_ view: YapWindowPointerTrackingView, context: Context) {
        view.onChange = onChange
        view.onKeyboardActivity = onKeyboardActivity
        view.refreshAttachment()
    }

    static func dismantleNSView(_ view: YapWindowPointerTrackingView, coordinator: ()) { view.stop() }
}

struct YapWindowPointerState {
...
@MainActor
final class YapWindowPointerTrackingView: NSView {
    ...
    private var state = YapWindowPointerState()
