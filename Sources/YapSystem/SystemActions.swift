...
public enum YapSystemAction: Equatable, Sendable {
    case openYap
...
struct PendingSystemActions: Sendable {
    private struct Item: Sendable {
        let action: YapSystemAction
        let requestedAt: Date
    }
    private var items: [Item] = []

    mutating func enqueue(_ action: YapSystemAction, now: Date = .now) {
    ...
    mutating func discardMeetingNavigation() {
        items.removeAll { item in
            ...
            case .openYap, .showUpcomingMeetings: false
            }
        }
    }

    mutating func take(now: Date = .now) -> YapSystemAction? {
...
@MainActor
public enum YapSystemActions {
    public static let notificationName = Notification.Name("YapSystemActionRequested")
    private static var pendingActions = PendingSystemActions()

    public static func request(_ action: YapSystemAction) {
    ...
    public static func takePendingAction() -> YapSystemAction? {
        pendingActions.take()
    }
...
public struct OpenYapIntent: AppIntent {
    public static let title: LocalizedStringResource = "Open Yap"
...
public struct ShowUpcomingMeetingsIntent: AppIntent {
    ...
    public static let description = IntentDescription("See your upcoming meetings in Yap.")
    public static let supportedModes: IntentModes = .foreground
    public init() {}

    @MainActor
    public func perform() async throws -> some IntentResult {
        YapSystemActions.request(.showUpcomingMeetings)
        return .result()
    }
}
...
public struct JoinNextMeetingIntent: AppIntent {
    ...
    @MainActor
    public func perform() async throws -> some IntentResult {
        YapSystemActions.request(.joinNextMeeting)
        return .result()
    }
}
...
public struct YapSystemIntentPackage: AppIntentsPackage {}
