...
struct AgendaRulesTests {
    ...
    @Test func deepLinkRequiresExplicitTrustedMeeting() {
        #expect(WhooshDeepLink.meetingURL(from: URL(string: "whoosh://join?url=https%3A%2F%2Fzoom.us%2Fj%2F12345678901")!)?.host == "zoom.us")
        #expect(WhooshDeepLink.meetingURL(from: URL(string: "whoosh://join?url=https%3A%2F%2Fzoom.us.evil.example%2Fj%2F12345678901")!) == nil)
        #expect(WhooshDeepLink.meetingURL(from: URL(string: "whoosh://join?url=https%3A%2F%2Fzoom.us%2Fj%2F12345678901&url=https%3A%2F%2Fzoom.us%2Fj%2F12345678902")!) == nil)
    }

    @Test func googleImportRejectsWebClients() {
        #expect(throws: (any Error).self) { try WhooshConfigurationStore.parseGoogle(Data(#"{"web":{"client_id":"test.apps.googleusercontent.com"}}"#.utf8)) }
        #expect(throws: (any Error).self) { try WhooshConfigurationStore.parseGoogle(Data(#"{"installed":{"client_id":"untrusted"}}"#.utf8)) }
    }
}
