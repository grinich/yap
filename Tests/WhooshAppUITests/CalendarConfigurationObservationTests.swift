...
struct CalendarConfigurationObservationTests {
    @Test func firstImportNotifiesTheSettingsSignInControl() async throws {
        let suite = "WhooshCalendarConfigurationObservation.\(UUID())"
        let preferences = try #require(UserDefaults(suiteName: suite))
        defer { preferences.removePersistentDomain(forName: suite) }
        let path = FileManager.default.temporaryDirectory.appendingPathComponent("WhooshGoogleImport-\(UUID()).json")
...
private struct ImportCalendarService: WhooshCalendarServing {
