...
struct WhooshMenuBarActionTests {
    @Test func idlePrimaryTogglesButContextOpenAndManualJoinAlwaysShow() async {
        let fixture = MenuBarFixture()
        defer { fixture.cleanUp() }
        await fixture.model.start()
        #expect(fixture.toggleActions.primaryAction == .openWhoosh)
        await fixture.toggleActions.performPrimaryAction(expectedAction: .openWhoosh)
        await fixture.toggleActions.performPrimaryAction(expectedAction: .openWhoosh)
        ...
        fixture.toggleActions.openWhoosh()
    ...
    @Test func queuedIdleToggleCannotBecomeAJoinOrHideANewCall() async {
        ...
        #expect(requestedAction == .openWhoosh)
    ...
    @Test func noCalendarMeetingOpensWhooshAndLeavesManualLinkEntryExplicit() async {
        let fixture = MenuBarFixture()
        defer { fixture.cleanUp() }
        await fixture.model.start()
        fixture.model.joinLink = "stale closed-sheet value"
        #expect(fixture.actions.primaryAction == .openWhoosh)
        #expect(fixture.actions.primaryActionTitle == "Zooom")
    ...
    @Test func floatingChatCanBeReopenedAndHiddenWithoutActivatingTheMainWindow() async {
        let fixture = MenuBarFixture()
        defer { fixture.cleanUp() }
        await fixture.connect()
        fixture.driver.setSharing(.sharing(MenuBarFixture.sharedWindow))
        fixture.model.sharingPresentation.setPresenting(true)
        fixture.model.sharingPresentation.chatVisible = false
        ...
        #expect(fixture.model.sharingPresentation.chatVisible)
        #expect(fixture.model.sharingPresentation.isPresenting)
    ...
    @Test func theMenuUsesTheCurrentChatSurfaceAndPreservesAnExplicitShowIntent() async {
        ...
        // The menu may have been opened before the floating panels appeared.
        fixture.model.sharingPresentation.setPresenting(true)
        fixture.model.sharingPresentation.chatVisible = true
        fixture.actions.setSharingChatVisible(true)
        #expect(fixture.model.sharingPresentation.chatVisible)
        #expect(fixture.model.sidebar == nil)
        #expect(fixture.openCount == 0)
...
private final class MenuBarFixture {
    let suite = "WhooshMenuBarTests.\(UUID())"
    ...
    var toggleCount = 0
    var now: Date
    lazy var actions = WhooshMenuBarActionHandler(model: model, now: { [weak self] in self?.now ?? .now }) { [weak self] in self?.openCount += 1 }
    lazy var toggleActions = WhooshMenuBarActionHandler(
    ...
    init(events: [CalendarEvent] = [], now: Date = .now) {
        ...
        model = WhooshModel(preview: false, preferences: preferences, meeting: MeetingCoordinator(driver: driver),
...
private actor MenuBarCalendar: WhooshCalendarServing {
