...
struct WhooshModelTests {
    ...
    @Test func permissionLostDuringBackgroundSyncDisablesWithoutGlobalAlert() async throws {
        ...
        fixture.reminders.synchronizeError = WhooshReminderPermissionError.notificationsNotAllowed
    ...
    @Test func importDisconnectsStoredCredentialsEvenBeforeConnectionStatusLoads() async throws {
        ...
        defer { fixture.cleanUp() }
        #expect(!fixture.model.isCalendarConnected)
        let path = FileManager.default.temporaryDirectory.appendingPathComponent("WhooshModelConfig-\(UUID()).json")
...
private final class ModelFixture {
    ...
    let suite = "WhooshModelTests.\(UUID())"
    ...
    func makeModel(preview: Bool = false) -> WhooshModel {
        let calendar = self.calendar
        let configuration = self.configuration
        return WhooshModel(preview: preview, preferences: preferences, meeting: liveMeeting, calendarClient: calendar,
...
private final class ModelReminders {
    ...
    var actions: WhooshReminderActions {
        WhooshReminderActions(requestAuthorization: { [self] in try await requestAuthorization() },
...
private actor ModelCalendar: WhooshCalendarServing {
