...
struct WhooshWindowDragSurfaceTests {
    ...
    @Test func headerButtonAndTextFieldReceiveTheirPressInsteadOfDragging() async throws {
        ...
        defer { fixture.cleanUp() }
        let surface = try await fixture.surface()
        let regions = fixture.hosting.descendants(of: WhooshWindowInteractionTrackingView.self)
...
private final class HeaderDragFixture {
    ...
    func surface() async throws -> WhooshWindowDragView {
        for _ in 0..<20 {
            hosting.layoutSubtreeIfNeeded()
            if let surface = hosting.descendants(of: WhooshWindowDragView.self).first { return surface }
            try await Task.sleep(for: .milliseconds(5))
        }
        return try #require(hosting.descendants(of: WhooshWindowDragView.self).first)
    }
    ...
    func cleanUp() {
        hosting.descendants(of: WhooshWindowInteractionTrackingView.self).forEach { $0.detach() }
