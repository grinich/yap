...
struct GoogleOAuthTests {
    ...
    @Test(.enabled(if: ProcessInfo.processInfo.environment["WHOOSH_TEST_LOOPBACK"] == "1"))
...
struct GoogleCalendarClientTests {
    let configuration = GoogleOAuthConfiguration(clientID: "whoosh-test.apps.googleusercontent.com")
    ...
    @Test func failedRefreshDoesNotReplaceCachedAgendaWithPartialResult() async throws {
        let folder = FileManager.default.temporaryDirectory.appendingPathComponent("WhooshCalendarTests-\(UUID())")
