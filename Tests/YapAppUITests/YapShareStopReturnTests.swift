...
struct YapShareStopReturnTests {
    @Test func acceptedStopKeepsBackgroundSurfacesUntilTheSDKConfirms() async {
        ...
        #expect(YapSharingOverlayLayout.shouldPresentChat(isPresenting: true, isSharing: meeting.sharing.isSharing, chatVisible: true))
        ...
        let foregroundPiP = YapSharingOverlayLayout.shouldPresent(
        ...
        #expect(!YapSharingOverlayLayout.shouldPresentChat(isPresenting: foregroundPiP, isSharing: meeting.sharing.isSharing, chatVisible: true))
        #expect(!consume(&transition, meeting))
    }
    ...
    @Test func anOrdinaryBackgroundCallShowsFacesWithoutOpeningFloatingChat() async {
        ...
        #expect(backgroundPiP(meeting))
        #expect(!YapSharingOverlayLayout.shouldPresentChat(isPresenting: true, isSharing: meeting.sharing.isSharing, chatVisible: true))
        let minimizedPiP = YapSharingOverlayLayout.shouldPresent(
            isConnected: meeting.isConnected, isPreview: false, hasMainWindow: true,
            applicationIsActive: true, mainWindowIsKey: true,
            mainWindowIsVisible: false, mainWindowIsMiniaturized: true)
        #expect(minimizedPiP)
        driver.emit(.sharing(.sharing(Self.target)))
        #expect(YapSharingOverlayLayout.shouldPresentChat(isPresenting: minimizedPiP, isSharing: meeting.sharing.isSharing, chatVisible: true))
        #expect(!YapSharingOverlayLayout.shouldPresentChat(isPresenting: minimizedPiP, isSharing: meeting.sharing.isSharing, chatVisible: false))
        #expect(backgroundPiP(meeting))
        await meeting.leave()
        #expect(!backgroundPiP(meeting))
        #expect(!YapSharingOverlayLayout.shouldPresentChat(isPresenting: false, isSharing: meeting.sharing.isSharing, chatVisible: true))
    }

    private static let target = ShareTarget(id: "fixture-window", title: "Fixture window", kind: .window)
    private func consume(_ transition: inout YapSharingStopTransition, _ meeting: MeetingCoordinator) -> Bool {
    ...
    private func backgroundPiP(_ meeting: MeetingCoordinator) -> Bool {
        YapSharingOverlayLayout.shouldPresent(
            isConnected: meeting.isConnected, isPreview: false, hasMainWindow: true,
