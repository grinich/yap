...
struct YapSharingOverlayTests {
    @Test func presentsOnlyForConnectedLiveMeetingAwayFromMainWindow() {
        #expect(YapSharingOverlayLayout.shouldPresent(isConnected: true, isPreview: false, hasMainWindow: true, applicationIsActive: true, mainWindowIsKey: false, mainWindowIsVisible: true, mainWindowIsMiniaturized: false))
        #expect(!YapSharingOverlayLayout.shouldPresent(isConnected: false, isPreview: false, hasMainWindow: true, applicationIsActive: true, mainWindowIsKey: false, mainWindowIsVisible: true, mainWindowIsMiniaturized: false))
        #expect(!YapSharingOverlayLayout.shouldPresent(isConnected: true, isPreview: true, hasMainWindow: true, applicationIsActive: true, mainWindowIsKey: false, mainWindowIsVisible: true, mainWindowIsMiniaturized: false))
        #expect(!YapSharingOverlayLayout.shouldPresent(isConnected: true, isPreview: false, hasMainWindow: false, applicationIsActive: true, mainWindowIsKey: false, mainWindowIsVisible: false, mainWindowIsMiniaturized: false))
        #expect(!YapSharingOverlayLayout.shouldPresent(isConnected: true, isPreview: false, hasMainWindow: true, applicationIsActive: true, mainWindowIsKey: true, mainWindowIsVisible: true, mainWindowIsMiniaturized: false))
    }

    @Test func retainedKeyFlagInInactiveApplicationStillPresentsOverlays() {
        #expect(YapSharingOverlayLayout.shouldPresent(
    ...
    @Test func hiddenChatReopensOnItsNewDisplayInsteadOfStaleCoordinates() {
        ...
        var placement = YapSharingChatPlacement()
    ...
    @Test func confirmedStopReturnsToSameConnectedMeetingExactlyOnce() {
        var transition = YapSharingStopTransition()
        let session = UUID()
    ...
    @Test func closesChatForCurrentShareButResetsOnNextShare() {
        let state = YapSharingPresentation()
        let session = UUID()
    ...
    @Test func stripUsesOnlySixUniqueVisibleRemoteParticipants() {
        ...
        #expect(YapSharingOverlayLayout.participants(from: duplicated).map(\.id) == (0..<6).map { "remote-\($0)" })
    }
    ...
    @Test func soloCallKeepsSelfInPictureInPictureWithCameraOffOrOn() {
        for cameraEnabled in [false, true] {
            ...
            let selected = YapSharingOverlayLayout.participants(from: [local, local])
            #expect(selected.map(\.id) == ["me"])
            #expect(selected.first?.isCameraEnabled == cameraEnabled)
        }
        #expect(YapSharingOverlayLayout.participants(from: []).isEmpty)
    }
    ...
    @Test func pictureInPictureSwitchesToOthersAndBackToSelfAsPeopleComeAndGo() {
        ...
        #expect(YapSharingOverlayLayout.participants(from: [local]).map(\.id) == ["me"])
        #expect(YapSharingOverlayLayout.participants(from: [local, guest]).map(\.id) == ["guest"])
        #expect(YapSharingOverlayLayout.participants(from: [local]).map(\.id) == ["me"])
    }
    ...
    @Test func largerPictureInPictureFitsPortraitAndLandscapeDisplays() {
        for screen in [CGRect(x: 0, y: 0, width: 640, height: 480),
            ...
            for ratios in [[16.0 / 9.0], [9.0 / 16.0], Array(repeating: 16.0 / 9.0, count: 6)] {
                let pip = YapSharingOverlayLayout.stripFrame(in: screen, aspectRatios: ratios)
                ...
                let clamped = YapSharingOverlayLayout.clampedStripFrame(resized, in: screen)
                #expect(screen.contains(clamped))
                #expect(clamped.size == resized.size)
            }
            #expect(screen.contains(YapSharingOverlayLayout.chatFrame(in: screen)))
        }
        let screen = CGRect(x: 0, y: 0, width: 1440, height: 900)
        let normal = YapSharingOverlayLayout.stripFrame(in: screen, aspectRatios: [16 / 9])
        #expect(normal.width >= 384 && normal.height >= 216)
        let portrait = YapSharingOverlayLayout.stripFrame(in: screen, aspectRatios: [9 / 16])
